...
public class Solution {
    ...
    public static void ourInterruptMethod() {
        for (Thread thread : threads) {
            thread.interrupt();
            System.out.println(thread.getName() + "isInterapted() - " + thread.isInterrupted());
    ...
    public static class Water implements Runnable {
        private volatile String sharedResource;  // без volatile c syncronizedв прнципе работает норм но ХЗ
        ...
        public void run() {
            ...
            boolean isCurrentThreadInterrupted = Thread.currentThread().isInterrupted();    //это в принципе не нужно так как sleep
            // выкидывает эксепшен и цикл вылетает через кэтч.
            String threadName = Thread.currentThread().getName();

            try {
                while (!isCurrentThreadInterrupted) {
                    synchronized (this) {           // добавил изменение sharedResource у объекта и без synchronized оно работает стремно 
                        System.out.println("Объект " + (sharedResource += threadName) + ", нить " + threadName);
                    }
                    Thread.sleep(1000);
                }
            } catch (InterruptedException e) {
            }
            System.out.println("Объект " + sharedResource + ", нить " + threadName + " is stoped!");
        }
    }
}