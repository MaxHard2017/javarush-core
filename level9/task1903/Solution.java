...
public class Solution {
    ...
    public static class IncomeDataAdapter implements Customer, Contact {
        ...
        public String getPhoneNumber() {
            ...
            String number = Integer.toString(data.getPhoneNumber()); // переводим все в стринг
            int numberLength = number.length(); // вычисляем длину
        
            String code = number.substring(0, numberLength - 7);    // вырезаем код
            int codeInt = Integer.parseInt(code);                              // переводим код в инт 
            code = String.format("%03d", codeInt); // выводим инт с лидирующими нулями в стринг
            
            number = number.substring(numberLength - 7);        
            number = number.substring(0, 3) + "-"
                    + number.substring(3, 5) + "-"
                    + number.substring(5);
            
                return "+38(" + code + ")" + number;                            //собираем номер
        }
